...
Run this locally on lab laptop.
"""
...
class IronSulfurMLTrainer:
    """ML training class for protein-only (radius-independent) datasets"""
        ...
        models = {
            ...
            'SVR': {
                ...
                'params': {
                    ...
                    'kernel': ['rbf', 'poly'],  # dropped 'poly' for less complexity
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        """Load feature data from a specific CSV file or find one in the data directory"""
        
        if hasattr(self, 'load_specific_file'):
            # Use the specific file provided
            feature_file = self.load_specific_file
            self.logger.info(f"Loading data from specific file: {feature_file}")
        else:
            # Look for any feature file (no radius in name)
            feature_files = list(self.data_dir.glob(f"*.csv"))
    ...
    def plot_learning_curves(self, X: pd.DataFrame, y: pd.Series, model_name: str):
    ...
    def plot_prediction_analysis(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        plt.savefig(self.output_dir / f'prediction_analysis_{model_name}.png',
                    dpi=300, bbox_inches='tight')
        plt.close()
    
    def train_models(self, n_splits_outer: int = 5, n_splits_inner: int = 3, n_repeats: int = 10) -> Dict:
        """Train all models using nested CV"""
        
        self.logger.info(f"\n{'='*50}")
        self.logger.info(f"Training models for protein-only dataset")
        ...
        for model_name, model_config in self.models.items():
            ...
            self.grid_search_params[f"{model_name}"] = model_config['params']
            ...
            for repeat in range(n_repeats):
                ...
                for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X_processed)):
                    ...
                    if model_config['params']:
                        ...
                        best_model_path = self.output_dir / f"best_models" / f"{model_name}.pkl"
                        ...
                        best_model_path = self.output_dir / "best_models" / f"{model_name}.pkl"
                        best_model_path.parent.mkdir(exist_ok=True)
                        model_info = {
                            'model': best_model,
                            'best_params': {},
            ...
            self.plot_learning_curves(X_processed, y_processed, model_name)
    ...
    def train_best_model_for_shap(self, model_name: str) -> Tuple[Any, pd.DataFrame, pd.Series, List[str]]:
        ...
        if model_config['params']:
            ...
            cv_results_df.to_csv(self.output_dir / f'grid_search_results_{model_name}.csv', index=False)
    ...
    def perform_shap_analysis(self, model_name: str, top_features: int = 20):
        """Perform SHAP analysis with dataset-specific title."""
        
        if not SHAP_AVAILABLE:
            self.logger.warning("SHAP not available, skipping feature importance analysis")
            return
        
        # Get dataset name from the specific CSV file path
        if hasattr(self, 'load_specific_file'):
            csv_filename = Path(self.load_specific_file).name
            dataset_name = Path(self.load_specific_file).stem
            
            # Create more readable dataset title
            if "all_cofactors" in dataset_name:
                dataset_title = "All Iron-Sulfur Cofactors"
            elif "FES" in dataset_name:
                dataset_title = "FES Cofactors Only"
            elif "SF4" in dataset_name:
                dataset_title = "SF4 Cofactors Only"
            else:
                dataset_title = dataset_name.replace("_", " ").title()
        else:
            # Fallback if no specific file is set
            dataset_title = "Unknown Dataset"
        
        self.logger.info(f"Performing SHAP analysis for {model_name} on {dataset_title}")
        
        try:
            # Train best model and get data
            best_model, X, y, feature_names = self.train_best_model_for_shap(model_name)
            ...
            # Create single SHAP plot for all features (no feature type grouping)
            plt.figure(figsize=(12, 10))
            shap.summary_plot(
                shap_values,
                X_transformed,
                feature_names=feature_names,
                max_display=min(top_features, len(feature_names)),
                show=False
            )
            plt.title(f'Feature Importance - {dataset_title}\n{model_name}', fontsize=14)
            plt.tight_layout()
            plt.savefig(
                self.output_dir / f'shap_summary_{model_name}.png',
                dpi=300,
                bbox_inches='tight'
            )
            plt.close()
            ...
            feature_importance = pd.DataFrame({
                ...
                'importance': np.abs(shap_values).mean(0) if isinstance(shap_values, np.ndarray) 
                            else np.abs(shap_values).mean(0)
            })
            ...
            self.logger.info(f"SHAP analysis completed for {model_name} on {dataset_title}")
    ...
    def run_analysis(self):
        """Run the complete ML analysis pipeline for protein-only dataset"""
        
        self.logger.info("Starting ML analysis for iron-sulfur protein-only dataset")
        
        # Train models just once - no radius loop!
        self.results = self.train_models()
        
        # Find top 3 models based on MAE
        ...
        self.logger.info(f"\nBest overall model: {best_model_info['model_name']}")
        self.logger.info(f"Best MAE: {best_model_info['mae']:.2f} mV")
        
        # Log all top 3 models
        self.logger.info("\nTop 3 performing models:")
        for i, model_info in enumerate(top_models, 1):
            self.logger.info(f"{i}. {model_info['model_name']} - MAE: {model_info['mae']:.2f} mV")
        
        # Perform SHAP analysis for top 3 models
        for model_info in top_models:
            model_name = model_info['model_name']
            self.logger.info(f"\nPerforming SHAP analysis for top model: {model_name}")
            self.perform_shap_analysis(model_name)
    ...
    def create_visualizations(self):
        """Create visualization plots for single-radius dataset"""
        ...
        # Plot MAE bars
        ax = plot_df.sort_values('MAE').plot(
            kind='barh', x='Model', y='MAE', 
            xerr='MAE_std', capsize=5, 
            color='skyblue', legend=False
        )
        
        # Add error values to the bars
        for i, (mae, std) in enumerate(zip(plot_df.sort_values('MAE')['MAE'], 
                                        plot_df.sort_values('MAE')['MAE_std'])):
            ax.text(mae + 0.5, i, f"{mae:.2f} ± {std:.2f}", va='center')
        
        plt.xlabel('Mean Absolute Error (mV)')
        plt.ylabel('Model')
        plt.title('Model Performance Comparison (MAE)')
        plt.grid(True, alpha=0.3, axis='x')
        plt.tight_layout()
        plt.savefig(self.output_dir / 'model_mae_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # Model Comparison Bar Chart for R²
        plt.figure(figsize=(12, 8))
        
        # Sort by R2 (descending)
        r2_sorted = plot_df.sort_values('R2', ascending=False)
        
        # Plot R2 bars
        ax = r2_sorted.plot(
            kind='barh', x='Model', y='R2', 
            xerr='R2_std', capsize=5, 
            color='lightgreen', legend=False
        )
        
        # Add error values to the bars
        for i, (r2, std) in enumerate(zip(r2_sorted['R2'], r2_sorted['R2_std'])):
            ax.text(max(0.01, r2 - 0.15), i, f"{r2:.3f} ± {std:.3f}", va='center')
        ...
        plt.grid(True, alpha=0.3, axis='x')
        plt.tight_layout()
        plt.savefig(self.output_dir / 'model_r2_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # Create a performance summary table
        metrics_table = plot_df.sort_values('MAE')[['Model', 'MAE', 'MAE_std', 'RMSE', 'RMSE_std', 'R2', 'R2_std']]
        
        # Format the table as an image
        ...
        # Create table
        table = ax.table(
            cellText=metrics_table.round(3).values,
...
def main():
    ...
    for feature_file in feature_files:
        # Extract dataset name from filename (without requiring "features" in the name)
        dataset_name = feature_file.stem
        
        # Create a specific output directory for this dataset
        ...
        # Initialize trainer with dataset-specific output directory
        ...
        # Modify the load_data method to use the specific feature file
        trainer.load_specific_file = feature_file
